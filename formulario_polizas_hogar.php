    ...
    <div id="principal" class="container">
      ...
      <?php
       if(isset($_POST['guardar_poliza'])){
        ...
        if($dirInmueble == ""){
          array_push($campos, "El campo direccion de inmueble no puede estar vacio.");
        }
        if($valor == "" || is_numeric($valor)== FALSE){
          array_push($campos, "El campo Nombre Parroquia no puede estar vacio.");
        }
        if($contenido == ""){
          array_push($campos, "El campo Contenido no puede estar vacio.");
        }
        if($riesgosAux == ""){
          array_push($campos, "El campo riesgos auxiliares no puede estar vacio.");
        }
        if($desTCober == ""){
          array_push($campos, "El campo Descripcion de Tipo de Cobertura no puede estar vacio.");
        }
    ...
    array_push($campos, "El tipo de interes no puede ser vacio.");
          ...
          if(count($campos)>0) {
              ...
              for($i=0; $i < count($campos); $i++){
                  ...
                  include ("add_poliza_hogar.php");
              }
              echo "</div>";
       }
      ?>
     <tr height="50"><td>Prima: <input type="text" name="txt_prima" onkeyup="this.value=this.value.toUpperCase()"></td></tr>
     <tr height="50"><td>Direccion del inmueble: <input type="text" name="txt_dirInmueble" onkeyup="this.value=this.value.toUpperCase()"></td></tr>
     <tr height="50">
       <td>
         Valor: <input type="text" name="txt_valor" onkeyup="this.value=this.value.toUpperCase()">
        </td></tr>
     <tr height="50"><td>Contenido: <input type="text" name="txt_contenido" onkeyup="this.value=this.value.toUpperCase()"></td></tr>
     <tr height="50"><td>Riesgos Auxiliares: <input type="text" name="txt_riesgosAux" onkeyup="this.value=this.value.toUpperCase()"></td></tr>
     <tr height="50"><td>Descripcion de tipo de cobertura: <input type="text" name="txt_desTCober" onkeyup="this.value=this.value.toUpperCase()"></td></tr>
